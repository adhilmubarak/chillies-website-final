...
const FlashSaleView: React.FC<FlashSaleViewProps> = ({ 
  ...
  useEffect(() => {
    const calculateTimeLeft = () => {
      ...
      const [h, m] = (flashSaleEndTime || '23:59').split(':').map(Number);
  ...
  return (
    <div className="w-full space-y-12 animate-fade-in">
      <div className="relative w-full h-80 md:h-[450px] overflow-hidden rounded-3xl border-4 border-red-600/50 bg-white group shadow-[0_0_60px_rgba(220,38,38,0.25)]">
        <div className="absolute inset-0 bg-red-600/5 animate-pulse z-20 pointer-events-none"></div>
        <div className="absolute inset-0 z-30 bg-gradient-to-r from-white via-transparent to-white pointer-events-none"></div>
        <div className="absolute inset-0 z-30 bg-gradient-to-t from-white via-transparent to-transparent pointer-events-none"></div>
        <div className="absolute inset-0 z-40 flex flex-col items-center justify-center gap-6 px-6 text-center">
            ...
            <div className="space-y-2">
                ...
                <h3 className="text-stone-900 text-3xl md:text-5xl font-serif">Unbeatable Deals <span className="text-red-500 italic">Ending Soon</span></h3>
            </div>
            <div className="flex flex-col items-center bg-black/60 backdrop-blur-2xl px-10 py-6 rounded-3xl border border-stone-900/10 shadow-2xl mt-4">
                <div className="flex items-center gap-3 mb-4">
                    <Timer size={18} className="text-red-500" />
                    <span className="text-[10px] md:text-xs text-stone-300 font-bold uppercase tracking-[0.3em]">Offer Ends In</span>
                </div>
                <div className="flex gap-4 md:gap-8 text-white font-mono text-3xl md:text-6xl font-bold tracking-wider">
                    <div className="flex flex-col items-center">
                        ...
                        <span className="text-[10px] text-stone-300 font-sans tracking-[0.2em] mt-1 uppercase">Hours</span>
                    </div>
                    <span className="text-red-600 animate-pulse">:</span>
                    <div className="flex flex-col items-center">
                        <span className="tabular-nums">{String(timeLeft.minutes).padStart(2, '0')}</span>
                        <span className="text-[10px] text-stone-300 font-sans tracking-[0.2em] mt-1 uppercase">Mins</span>
                    </div>
                    <span className="text-red-600 animate-pulse">:</span>
                    <div className="flex flex-col items-center">
                        <span className="tabular-nums text-red-500">{String(timeLeft.seconds).padStart(2, '0')}</span>
                        <span className="text-[10px] text-stone-300 font-sans tracking-[0.2em] mt-1 uppercase">Secs</span>
                    </div>
                </div>
            </div>
        </div>
        <div className="flex absolute top-0 left-0 h-full animate-marquee-fast grayscale-[0.5] opacity-40 hover:grayscale-0 hover:opacity-100 transition-all duration-700">
            {marqueeItems.map((item, idx) => (
                <div key={`${item.id}-${idx}`} className="h-full w-56 md:w-[400px] flex-shrink-0 relative">
                    <img src={item.image || undefined} alt={item.name} className="w-full h-full object-cover border-x border-stone-900/5"/>
            ...
            return (
                <div key={item.id} className="relative group">
                    ...
                    <div className="ring-2 ring-red-900/30 rounded-3xl h-full transition-all hover:ring-red-500/50 hover:shadow-[0_0_30px_rgba(220,38,38,0.2)]">
                        <MenuItemCard 
                            item={item} onAdd={onAdd} index={index} 
