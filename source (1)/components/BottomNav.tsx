...
const BottomNav: React.FC<BottomNavProps> = ({ cartItemCount, onOpenCart, onOpenTracker, activeSection }) => {
  ...
  const navItemClass = (id: string) => `
    ...
    ${activeSection === id ? 'text-brand-400' : 'text-stone-500'}
  `;

  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-[100] bg-white/90 backdrop-blur-2xl border-t border-stone-900/5 pb-safe shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
      <div className="flex items-center justify-around h-16 px-2">
        <button onClick={() => scrollTo('home')} className={navItemClass('home')}>
          <Home size={20} className={activeSection === 'home' ? 'fill-brand-400/10' : ''} />
        ...
        <div className="relative -mt-8">
            <button 
                onClick={onOpenCart}
                className="w-14 h-14 bg-brand-500 rounded-full flex items-center justify-center text-white shadow-[0_8px_20px_rgba(249,115,22,0.4)] border-4 border-white active:scale-90 transition-transform"
            >
                <ShoppingBag size={24} />
                {cartItemCount > 0 && (
