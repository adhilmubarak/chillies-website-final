...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ 
  ...
  return (
    <div className={`group flex flex-col h-full bg-stone-50 border border-stone-200 rounded-3xl overflow-hidden transition-all duration-500 ${isItemInteractable ? 'hover:border-brand-500/50 hover:shadow-[0_20px_40px_rgba(0,0,0,0.6)]' : 'opacity-80'}`}>
      <div className="relative h-60 sm:h-72 w-full overflow-hidden">
        ...
        {item.isUnavailable && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-[2px] flex flex-col items-center justify-center text-center p-4 z-20">
                ...
                <span className="text-stone-600 font-medium text-[9px] uppercase tracking-widest">Available Soon</span>
            </div>
        )}

        {isTimeRestricted && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-[3px] flex flex-col items-center justify-center text-center p-4 z-20 animate-fade-in">
                <div className="bg-brand-500/10 border border-brand-500/40 text-brand-500 px-5 py-2.5 rounded-2xl flex flex-col items-center gap-2 shadow-2xl">
                    <Clock size={20} className="animate-pulse" />
                    <div className="space-y-0.5">
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] block">Available From</span>
                        <span className="text-sm font-serif italic font-bold text-stone-900 block">
        ...
        {!isStoreOpen && !item.isUnavailable && !isTimeRestricted && (
            <div className="absolute inset-0 bg-white/60 backdrop-blur-sm flex flex-col items-center justify-center text-center p-4 z-20">
                <Lock className="text-stone-600/50 mb-2" size={24} />
                <span className="text-stone-600 font-bold uppercase tracking-[0.2em] text-[9px]">Kitchen Closed</span>
            </div>
        )}

        <div className="absolute top-4 left-4 flex flex-wrap gap-2 z-10 max-w-[90%]">
            {item.isChefChoice && <div className="bg-brand-500 text-white p-2 rounded-full shadow-lg border border-brand-300/30"><ChefHat size={12} /></div>}
            {item.isVegetarian && <div className="bg-white/80 backdrop-blur-sm text-green-400 p-2 rounded-full border border-green-500/30"><Leaf size={12} /></div>}
            {renderSpicyTag()}
        </div>

        <div className="absolute top-4 right-4 z-10">
            <div className={`bg-white/80 backdrop-blur-md font-bold px-3 py-1.5 rounded-xl border border-stone-900/10 text-sm transition-colors duration-300 ${!isItemInteractable ? 'text-stone-600' : 'text-brand-400'}`}>
                ₹{displayPrice}
            </div>
        </div>
      </div>
      ...
      <div className="flex flex-col flex-grow p-5 md:p-7">
        <h3 className={`font-serif text-lg md:text-xl font-medium transition-colors line-clamp-1 mb-2 ${isItemInteractable ? 'text-stone-900 group-hover:text-brand-400' : 'text-stone-600'}`}>
            {item.name}
        </h3>
        ...
        <button
          ...
          className={`w-full py-4 rounded-xl font-black uppercase text-[10px] tracking-[0.2em] transition-all flex items-center justify-center gap-2 ${
              ...
              isItemInteractable ? 'bg-stone-100 text-stone-900 hover:bg-brand-500 hover:text-white hover:shadow-[0_10px_20px_rgba(249,115,22,0.2)]' : 
              'bg-white text-stone-700 cursor-not-allowed border border-stone-200/50'
          }`}
        >
