  ...
  return (
    <section className="pb-20 pt-8 relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-stone-50">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-brand-500/10 via-stone-50 to-white"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
        <div className="text-center mb-16">
          <div className="inline-flex items-center justify-center p-3 bg-brand-500/10 rounded-full border border-brand-500/20 mb-4 animate-fade-in">
            <ChefHat className="text-brand-500 w-8 h-8" />
          </div>
          <h2 className="text-3xl md:text-5xl font-serif text-stone-900 mb-4">
            Chef's <span className="text-brand-500 italic">Masterpieces</span>
          </h2>
          <p className="text-stone-600 font-light max-w-xl mx-auto flex items-center justify-center gap-2">
          ...
          {items.map((item, index) => {
            ...
            return (
              <div key={item.id} className="relative group">
                 ...
                 <div className="absolute inset-0 bg-brand-500/20 blur-2xl rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 -z-10"></div>
                 
                 <div className="relative transform transition-transform duration-500 hover:-translate-y-2 h-full">
                    {/* Badge */}
                    <div className="absolute -top-3 -right-3 z-30 bg-brand-500 text-white px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest shadow-lg flex items-center gap-1">
